...
{
    /// <summary>
    /// Attempt to find a .csv file on the file system. This function will 
    /// return a string array containing the file name (index 0) and the entire 
    /// path to this file (index 1). If this function tries to return a file 
    /// that does not end with .csv, the return value will be a single array 
    /// value containing the word "error"
    /// </summary>
    public static string[] SelectCSVFile()
    {
        string path = EditorUtility
                      .OpenFilePanel("Select .CSV File", "", "csv");
        ...
        if (path.Length != 0)
        {
            ...
            // I had a weird error, so I added this extra measure to get the 
            // file correctly
            if (IsInMacOS)
            {
                Debug.Log("MAC");
